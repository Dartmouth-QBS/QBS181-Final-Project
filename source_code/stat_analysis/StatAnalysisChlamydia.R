...
AnovaTukey<-function(x){
  model<-aov(avg~Comm_Group,data=chlamydia_pivot %>% filter(year==x) %>%
...
tukey_results_clm = apply(p_vals, MARGIN = 2, function(x) {
  ifelse(x < 0.001, "< 0.001", round(x, 5))
})

tukey_results_clm = format_kable(tukey_results_clm)
